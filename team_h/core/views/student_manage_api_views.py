...
from ..models import Class, School, StudentUser
...
def get_classes(request):
    ...
    logger.debug("school_id: %s", school_id)
...
def get_student(request):
    ...
    try:
        student = StudentUser.objects.get(user_id=int(student_id), school=school)
...
def receive_form(request):
    ...
    form = StudentRegisterForm(form_data, school_id=school.id)

    if not form.is_valid():
        logger.debug("REGISTER ERROR: %s", form.errors)
    ...
    class_ids = [int(i) for i in data.get("class_ids", [])]
...
def update_student(request, student_id):
    ...
    if "birthdate" in data and data["birthdate"]:
        try:
            student.birthdate = datetime.datetime.strptime(data["birthdate"], "%Y-%m-%d").date()
        except ValueError:
            return JsonResponse({"errors": {"birthdate": ["日付形式が正しくありません"]}}, status=400)

    if "user_password" in data:
        password = str(data["user_password"])
        if len(password) < 8 or len(password) > 128:
            return JsonResponse({
                "errors": {"user_password": ["パスワードは8文字以上128文字以下で入力してください"]}
            }, status=400)
...
def api_student_create(request):
    ...
    return JsonResponse({"id": student.id, "result": "ok"})

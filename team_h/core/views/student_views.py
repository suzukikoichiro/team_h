...
def student_delete_by_admin(request, student_id):
    student = get_object_or_404(StudentUser, id=student_id, school_id=request.session.get("school_id"))
    student.delete()
...
def student_delete_by_teacher(request, student_id):
    student = StudentUser.objects.filter(id=student_id, school=request.teacher.school).first()
    if student is None:

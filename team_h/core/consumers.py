...
class WorldConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        logger.debug(
            "WORLD CONNECT TRY path_school=%s user=%s",
            self.scope["url_route"]["kwargs"]["school_id"],
            self.scope.get("user"),
        )
        ...
        await self.send(text_data=json.dumps({
            "type": "init",
            "players": WORLD_STATE.setdefault(self.school_id, {}),
        }))
    ...
    async def disconnect(self, close_code):
        uid = self.user.id if self.user and self.user.is_authenticated else self.user_id
        logger.debug("WORLD DISCONNECT user=%s code=%s", uid, close_code)
        ...
        if uid is not None:
            WORLD_STATE.setdefault(self.school_id, {}).pop(uid, None)
            await self.channel_layer.group_send(
                self.group_name,
                {
                    "type": "player_leave",
    ...
    async def receive(self, text_data):
        try:
            ...
            logger.warning("WS JSON ERROR: %s payload=%s", e, text_data)
            return

        msg_type = data.get("type")
        logger.debug("WS RECEIVE: %s", data)
        ...
        if msg_type == "move":
            ...
            if user_id is None or x is None or y is None:
                logger.warning("INVALID MOVE PACKET: %s", data)
                return
            ...
            player = room.setdefault(user_id, {})
            player.update(data)
            player["type"] = "move"
